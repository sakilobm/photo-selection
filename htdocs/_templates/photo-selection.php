...
<div id="authView" class="app-view active min-h-screen flex items-center justify-center p-4 relative z-10">
    <div class="glass-panel w-full max-w-md rounded-[2rem] p-8 md:p-10 shadow-2xl relative overflow-hidden border border-white/10">
        <!-- Light Flare Overlay -->
        <div class="absolute -top-32 -right-32 w-64 h-64 bg-[var(--theme-accent)] opacity-20 rounded-full blur-[80px] transition-colors duration-500"></div>

        <div class="text-center mb-8 relative">
            <div class="inline-flex p-3 rounded-2xl bg-white/5 border border-white/10 mb-4 shadow-inner">
                <i data-lucide="aperture" class="w-10 h-10 text-[var(--theme-accent)] animate-spin-slow transition-colors duration-500"></i>
            </div>
            <h1 class="text-3xl font-extrabold tracking-wider text-white font-['Outfit']">OBM <span class="text-[var(--theme-accent)] transition-colors duration-500">STUDIO</span></h1>
        ...
        <div class="mt-6 text-center">
            <a href="<?= Session::url('index') ?>" class="inline-flex items-center gap-2 text-xs text-gray-400 hover:text-white transition-colors">
...
<div id="galleryView" class="app-view flex-col min-h-screen z-10 relative hidden">
    ...
    <header class="glass-panel sticky top-4 z-40 mx-4 md:mx-6 my-4 rounded-2xl border border-white/10 shadow-lg px-6 py-4 transition-all duration-300">
        <div class="max-w-7xl mx-auto flex flex-col md:flex-row items-center justify-between gap-4">
            <div class="flex items-center gap-3">
                ...
                <div>
                    <h2 class="text-xl font-extrabold tracking-wider text-white">OBM <span class="text-[var(--theme-accent)] transition-colors">STUDIO</span></h2>
                    <p class="text-[9px] text-gray-400 tracking-widest uppercase">Premium Portal</p>
                </div>
            </div>

            <!-- Client Profile Badge -->
            <div class="flex items-center gap-3 bg-white/5 border border-white/10 rounded-2xl px-4 py-2 hover:bg-white/10 transition-colors shadow-inner shrink-0">
                <div class="relative shrink-0">
                    <div class="absolute -inset-0.5 rounded-full bg-gradient-to-tr from-[var(--theme-accent)] to-transparent opacity-75 blur-[1px]"></div>
                    <img src="https://images.unsplash.com/photo-1534528741775-53994a69daeb?auto=format&fit=crop&w=150&h=150&q=80" alt="Client Avatar" class="w-8 h-8 rounded-full object-cover relative border border-white/10">
                    <span class="absolute bottom-0 right-0 w-2.5 h-2.5 rounded-full bg-emerald-500 border-2 border-[#0f172a] flex items-center justify-center">
                        <span class="absolute w-full h-full rounded-full bg-emerald-500 opacity-75 animate-ping"></span>
                    </span>
                </div>
                <div class="text-left leading-tight">
                    <span class="block text-[8px] text-gray-400 font-bold uppercase tracking-widest">Active Session</span>
                    <span id="clientNameDisplay" class="block text-xs font-bold text-white tracking-wide">Valued Client</span>
                </div>
            </div>
            ...
            <div class="flex items-center flex-wrap gap-4">
                ...
                <div class="flex items-center gap-1.5 bg-white/5 border border-white/10 p-1 rounded-xl">
                    ...
                    <button onclick="setAppTheme('sapphire', event)" class="w-6 h-6 rounded-full bg-sky-500 border-2 border-white/20 hover:scale-110 transition-transform" title="Sapphire Ice"></button>
                    <button onclick="setAppTheme('amethyst', event)" class="w-6 h-6 rounded-full bg-purple-500 border-2 border-white/20 hover:scale-110 transition-transform" title="Amethyst Glow"></button>
                    <button onclick="setAppTheme('emerald', event)" class="w-6 h-6 rounded-full bg-emerald-500 border-2 border-white/20 hover:scale-110 transition-transform" title="Emerald Forest"></button>
                    <button onclick="setAppTheme('rose', event)" class="w-6 h-6 rounded-full bg-rose-500 border-2 border-white/20 hover:scale-110 transition-transform" title="Rose Quartz"></button>
                    <button onclick="setAppTheme('amber', event)" class="w-6 h-6 rounded-full bg-amber-500 border-2 border-white/20 hover:scale-110 transition-transform" title="Amber Sunset"></button>
                    <button onclick="setAppTheme('white', event)" class="w-6 h-6 rounded-full bg-white border-2 border-white/20 hover:scale-110 transition-transform" title="Pearl White"></button>
                </div>
                ...
                <div class="flex gap-1.5">
                    <button onclick="triggerResetGallery()" class="p-2.5 bg-white/5 hover:bg-white/10 text-gray-300 hover:text-white rounded-xl border border-white/10 transition-colors" title="Reset Gallery Selections & Deletions">
                        <i data-lucide="rotate-ccw" class="w-4.5 h-4.5"></i>
                    </button>
                    <button onclick="logout()" class="p-2.5 bg-red-500/10 hover:bg-red-500/20 text-red-400 hover:text-red-300 rounded-xl border border-red-500/20 transition-colors" title="Logout">
    ...
    <main class="max-w-7xl w-full mx-auto px-4 md:px-6 py-6 flex-grow flex flex-col gap-8">
        <div class="flex flex-col lg:flex-row lg:items-end justify-between gap-6 mb-2">
            <div class="space-y-3">
                <div class="flex items-center gap-3">
                ...
                <p class="text-sm text-slate-400 max-w-xl">
                    Experience every moment in a luxury photo gallery. Review your captures, filter by event categories, select your favorites, and finalize your list.
                </p>
                <div class="flex items-center gap-4 pt-1">
                    ...
                    <div class="flex items-center gap-2 text-[11px] text-slate-500">
                        ...
                        <span>Passcode: <strong class="text-[var(--theme-accent)] font-mono" id="clientMetaCode">ADMIN2026</strong></span>
                    </div>
                </div>
            </div>
        </div>
        ...
        <section id="carouselSection" class="w-full relative">
            <div class="flex items-center justify-between mb-3 px-1">
                ...
                <div class="flex items-center gap-1">
                    <button onclick="slideCarouselPrev()" class="p-1.5 rounded-lg bg-white/5 border border-white/10 hover:bg-white/10 text-gray-400 hover:text-white transition-all">
                        <i data-lucide="chevron-left" class="w-4 h-4"></i>
                    </button>
                    <button onclick="slideCarouselNext()" class="p-1.5 rounded-lg bg-white/5 border border-white/10 hover:bg-white/10 text-gray-400 hover:text-white transition-all">
            ...
            <div class="relative w-full overflow-hidden rounded-[1.75rem] glass-panel p-1.5">
                <div class="w-full overflow-hidden rounded-2xl relative">
                    <div id="carouselTrack" class="flex w-full transition-transform duration-500 ease-out cursor-grab active:cursor-grabbing slider-wrapper" style="transform: translateX(0%);">
                        <!-- Carousel Slides injects dynamically -->
                    </div>
                </div>
                <!-- Indicator dots -->
                <div id="carouselDots" class="absolute bottom-6 left-1/2 -translate-y-1/2 -translate-x-1/2 flex gap-1.5 bg-black/40 backdrop-blur-md px-3 py-1.5 rounded-full border border-white/10"></div>
            </div>
        </section>

        <!-- 2. SEARCH, CATEGORIES & INTERACTION CONTROL BAR -->
        <section id="filtersSection" class="flex flex-col lg:flex-row items-stretch lg:items-center justify-between gap-4 bg-white/5 p-4 rounded-2xl border border-white/10 glass-panel">
            ...
            <div class="flex-grow max-w-md relative flex items-center">
                <input type="text" id="gallerySearchInput" oninput="handleSearch(this.value)" placeholder="Search photos by filename..." class="w-full glass-input rounded-xl py-2.5 px-4 pl-10 text-white text-xs focus:outline-none focus:border-[var(--theme-accent)] transition-all">
        ...
        <section id="actionToolbar" class="hidden flex-wrap items-center justify-between gap-4 p-4 rounded-xl border border-white/10 bg-black/40 backdrop-blur-lg">
            <div class="flex items-center gap-3">
                <i data-lucide="settings-2" class="w-5 h-5 text-[var(--theme-accent)]"></i>
                <p class="text-sm text-gray-300"><span id="toolbarSelectedCount" class="font-bold text-white">0</span> items selected</p>
            </div>
            <div class="flex items-center gap-2">
                <button onclick="bulkDeselect()" class="text-xs font-medium text-gray-400 hover:text-white px-3 py-2 rounded-lg bg-white/5 border border-white/10 transition-colors">Clear Selection</button>
                <button onclick="bulkDelete()" class="text-xs font-semibold text-red-400 hover:text-red-300 px-3 py-2 rounded-lg bg-red-500/10 border border-red-500/20 transition-colors">Delete Selected</button>
            </div>
        </section>
...
<!-- ========================================== -->
<div id="lightboxModal" class="fixed inset-0 z-50 bg-black/95 backdrop-blur-xl hidden flex-col items-center justify-center p-4 select-none">
    ...
    <div class="absolute top-6 left-1/2 -translate-x-1/2 z-20 pointer-events-auto">
        <div class="px-4 py-2 rounded-full bg-black/50 border border-white/10 backdrop-blur-md flex items-center gap-2.5">
            <p id="lightboxFilename" class="text-xs font-bold text-gray-200 truncate max-w-[150px] sm:max-w-xs">IMG_0001.jpg</p>
    ...
    <!-- Swipe Navigators -->
    <button onclick="navigateLightbox(-1)" class="absolute left-6 z-20 hidden md:flex p-4 rounded-2xl bg-white/5 border border-white/10 text-gray-400 hover:text-white hover:scale-105 active:scale-95 transition-all" title="Previous Image">
        <i data-lucide="chevron-left" class="w-6 h-6"></i>
    </button>
    <button onclick="navigateLightbox(1)" class="absolute right-6 z-20 hidden md:flex p-4 rounded-2xl bg-white/5 border border-white/10 text-gray-400 hover:text-white hover:scale-105 active:scale-95 transition-all" title="Next Image">
    ...
    <!-- Thumbnails bar -->
    <div class="absolute bottom-24 left-4 right-4 z-10 hidden sm:flex justify-center gap-2 overflow-x-auto py-2 px-4 bg-black/40 backdrop-blur-md border border-white/10 rounded-2xl max-w-xl mx-auto" id="lightboxThumbs"></div>

    <!-- Lightbox Bottom Controls Capsule -->
    <div class="absolute bottom-6 left-1/2 -translate-x-1/2 z-20 pointer-events-auto flex items-center gap-3.5 bg-black/50 border border-white/10 backdrop-blur-md px-5 py-3 rounded-2xl shadow-2xl">
        <button onclick="navigateLightbox(-1)" class="p-2.5 rounded-xl bg-white/5 border border-white/10 text-gray-400 hover:text-white active:scale-95 transition-all md:hidden" title="Previous">
            <i data-lucide="chevron-left" class="w-4 h-4"></i>
        </button>

        <button id="lightboxSelectBtn" class="flex items-center gap-1.5 px-4 py-2.5 rounded-xl text-xs font-bold bg-white/5 border border-white/10 text-white hover:bg-white/10 active:scale-95 transition-all"></button>

        <button onclick="lightboxDeleteCurrent()" class="p-2.5 rounded-xl bg-red-500/10 border border-red-500/20 text-red-400 hover:bg-red-500 hover:text-white hover:bg-red-500 hover:border-red-500 transition-all" title="Delete Photo">
        ...
        <button onclick="closeLightbox()" class="p-2.5 rounded-xl bg-white/5 border border-white/10 text-gray-400 hover:text-white hover:bg-white/10 active:scale-95 transition-all" title="Close Preview">
            <i data-lucide="x" class="w-4 h-4"></i>
        </button>

        <button onclick="navigateLightbox(1)" class="p-2.5 rounded-xl bg-white/5 border border-white/10 text-gray-400 hover:text-white active:scale-95 transition-all md:hidden" title="Next">
            <i data-lucide="chevron-right" class="w-4 h-4"></i>
        </button>
    </div>
</div>
...
<div id="submitModal" class="fixed inset-0 z-[60] bg-black/90 backdrop-blur-xl hidden items-center justify-center p-4">
    ...
    <div class="glass-panel w-full max-w-lg rounded-[2.5rem] p-8 md:p-10 shadow-2xl border border-white/15 relative z-10 text-center max-h-[90vh] flex flex-col justify-between">
        <div>
            <div class="inline-flex p-4 rounded-full bg-emerald-500/10 border border-emerald-500/20 text-emerald-400 mb-6">
                <i data-lucide="send" class="w-12 h-12"></i>
            </div>
            <h3 class="text-3xl font-extrabold text-white mb-2">Finalize Selections</h3>
            <p class="text-sm text-gray-400">Please review your selected list below before sending to OBM Photo Studio.</p>

            <div class="my-6 max-h-48 overflow-y-auto bg-black/30 border border-white/10 rounded-xl p-4 text-left space-y-2.5 scrollbar-thin" id="submitSelectedList"></div>
        </div>
...
<!-- ========================================== -->
<div id="successOverlay" class="fixed inset-0 z-[70] bg-black/95 backdrop-blur-xl hidden flex-col items-center justify-center p-4">
    <div id="successConfetti" class="absolute inset-0 pointer-events-none overflow-hidden z-0"></div>
    <div class="glass-panel max-w-md w-full rounded-[2.5rem] p-8 md:p-10 border border-white/10 text-center relative z-10 flex flex-col items-center justify-center gap-6 animate-viewEnter">
        <div class="relative flex items-center justify-center">
            ...
            <div class="w-20 h-20 rounded-full bg-emerald-500/20 border-2 border-emerald-500 text-emerald-400 flex items-center justify-center relative shadow-lg shadow-emerald-500/20">
        ...
        <div>
            ...
            <p class="text-xs text-gray-400 uppercase tracking-widest font-bold mt-1 text-[var(--theme-accent)]">Selections Dispatched</p>
            <p class="text-sm text-gray-300 mt-4 leading-relaxed">
                Your selection of <span id="successPhotoCount" class="font-bold text-white">0</span> photos has been successfully transmitted directly to the OBM Photo Studio database.
            </p>
            <p class="text-xs text-gray-400 mt-2 leading-relaxed">Our editing team has been notified and will start preparing your album draft layout immediately.</p>
        </div>

        <button onclick="closeSuccessOverlay()" class="w-full bg-white/5 hover:bg-white/10 border border-white/10 text-white font-bold py-3.5 rounded-xl transition-all duration-300 active:scale-95 flex items-center justify-center gap-2 mt-4 custom-cursor-hide">
...
<!-- ========================================== -->
<div id="loadingOverlay" class="fixed inset-0 z-[70] bg-black/95 backdrop-blur-xl hidden flex-col items-center justify-center p-4 select-none">
    <div class="glass-panel max-w-md w-full rounded-[2.5rem] p-8 md:p-10 border border-white/10 relative z-10 flex flex-col gap-6 animate-viewEnter">
        <div class="text-center">
            ...
            <p class="text-xs text-gray-400 uppercase tracking-widest font-bold mt-1 text-[var(--theme-accent)]">Uploading selections to OBM cloud</p>
        </div>
        ...
        <div class="space-y-1.5">
            ...
            <div class="w-full h-2.5 bg-white/5 rounded-full overflow-hidden border border-white/5">
                <div id="globalUploadProgressBar" class="h-full bg-[var(--theme-accent)] rounded-full transition-all duration-300 ease-out" style="width: 0%;"></div>
            </div>
        </div>

        <div class="bg-black/30 border border-white/10 rounded-2xl p-4 flex flex-col gap-2">
            <div class="flex items-center justify-between text-xs">
                <span id="currentItemName" class="text-gray-200 font-semibold truncate max-w-[200px]">Initializing connection...</span>
                <span id="currentItemPercent" class="text-[var(--theme-accent)] font-bold">0%</span>
            </div>
            <div class="w-full h-1 bg-white/10 rounded-full overflow-hidden">
                <div id="currentItemProgressBar" class="h-full bg-emerald-500 rounded-full transition-all duration-100" style="style: 0%;"></div>
            </div>
        </div>

        <div class="space-y-2">
            <p class="text-[10px] uppercase font-bold text-gray-500 tracking-wider pl-1">Activity Log</p>
            <div id="uploadLogContainer" class="h-36 overflow-y-auto bg-black/40 border border-white/10 rounded-xl p-3.5 space-y-2 text-xs scrollbar-thin scroll-smooth text-left text-gray-300"></div>
        </div>
    </div>
</div>
...
<!-- ========================================== -->
<div id="portalLoadingScreen" class="fixed inset-0 z-[70] bg-black/95 backdrop-blur-xl hidden flex-col items-center justify-center p-4 select-none">
    <div class="glass-panel max-w-md w-full rounded-[2.5rem] p-8 md:p-10 border border-white/10 relative z-10 flex flex-col gap-6 animate-viewEnter text-center">
        <div class="relative flex items-center justify-center mx-auto mb-2">
            ...
            <div class="w-16 h-16 rounded-full bg-white/5 border border-white/10 flex items-center justify-center relative shadow-lg">
        ...
        <div>
            ...
            <p class="text-xs text-gray-400 uppercase tracking-widest font-bold mt-1 text-[var(--theme-accent)]" id="portalLoadingStatus">Connecting to OBM Core Server</p>
            <div class="mt-3 flex items-center justify-center gap-2 bg-white/5 border border-white/5 rounded-xl px-4 py-1.5 max-w-xs mx-auto">
        ...
        <div class="space-y-1.5">
            ...
            <div class="w-full h-2 bg-white/5 rounded-full overflow-hidden border border-white/5">
                <div id="portalProgressBar" class="h-full bg-[var(--theme-accent)] rounded-full transition-all duration-300 ease-out" style="width: 0%;"></div>
            </div>
        </div>

        <div class="bg-black/30 border border-white/5 rounded-2xl p-4 text-left space-y-2.5 text-xs font-mono text-gray-400">
...
<div id="customCursor" class="fixed pointer-events-none z-[999] w-12 h-12 rounded-full border border-white/20 bg-black/45 backdrop-blur-md text-[var(--theme-accent)] flex items-center justify-center opacity-0 scale-50 transition-all duration-300 ease-out shadow-2xl -translate-x-1/2 -translate-y-1/2 pointer-events-none">
