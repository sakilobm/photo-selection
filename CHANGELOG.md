...
- **Floating Pill Navigation Bar Layout & Whitespace Fix** — Fixed floating navbar pill clipping, text wrapping, and right-edge button overflow across all pages:
  - Added `white-space: nowrap !important` and `shrink-0` to all nav links and action buttons (`.nav-link`, `.btn-primary`, `.global-theme-switcher`).
  - Increased `.floating-nav-pill` maximum width from `1100px` to `1380px` for generous flex breathing room on desktop viewports.
  - Adjusted link font sizes (`0.825rem`) and padding (`6px 12px`) to eliminate two-line text wrapping on links like `Our Story`, `Live Event`, `Packages ₹`, `Admin ⚙️`, and `Client Portal`.
  - Added responsive scaling rules for laptop viewports (`1024px` - `1280px`).
