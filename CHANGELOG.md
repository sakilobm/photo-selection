...
- **Admin Passcode Lock Screen Light Theme Fix (`admin.html`)** — Added explicit `html.theme-light` CSS overrides for `.admin-lock-modal`, `.admin-lock-card`, and `.admin-lock-hint-box`. In Pearl White mode, the admin passcode lock screen modal background now renders as a clean frosted light overlay (`rgba(248,250,252,0.94)`), the passcode card as a crisp liquid white glass panel (`rgba(255,255,255,0.95)`), and titles/subtext in high-contrast dark slate (`#0f172a` / `#475569`).
