...
- **Icon-First Smooth Hover-Expand Navigation Bar Upgrade & Photo Selection Integration** — Upgraded website floating navigation bar to an interactive icon-first hover expand system:
  - Added **Photo Selection** portal (`photo-selection.html`) to navigation links across all pages (`index.html`, `packages.html`, `digital-album.html`, `live-event.html`, `photo-selection.html`, `admin.html`).
  - **Smooth Hover Animation**: In resting state, each link displays a vector icon. Upon hover or active selection, the text title expands with a smooth cubic-bezier transition (`max-width: 140px`, `opacity: 1`, `transform: translateX(0)`).
  - Active page links stay expanded with accent color glow indicators so users always know their active location.
